...
try {
    ...
    function mostrarTemas()
    {
        ...
        $sentencia = "SELECT A.imagen_artista, A.nombre_artistico_artista, T.nombre_tema, T.completo_tema, T.categoria_tema, T.nota_tema, T.imagen_tema FROM licencia L, artista A, tema T WHERE L.uuid_tema_licencia=T.uuid_tema AND L.uuid_usuario_licencia='$uuid' AND T.uuid_artista_tema=A.uuid_artista ORDER BY A.nombre_artistico_artista";
        $resultado = mysqli_query($conexion, $sentencia);
        $num_resultados = mysqli_num_rows($resultado);
        echo '
        <fieldset>
            <legend>MyLicenses</legend>';
        if ($num_resultados != 0) {
            echo '
            <table>
                <tr> 
                    <td class="verde"; colspan=7>RESULTADO/S: ', $num_resultados, ' LICENCIA/S DISPONIBLE/S</td>
                </tr>
                <tr>
                    <th class="verde";>IMAGEN ART.</th>
                    <th class="verde";>NOMBRE ART.</th>
                    <th class="verde";>NOMBRE TEM.</th>
                    <th class="verde";>MP3 COMPLETO</th>
                ...
                foreach ($array as $indice => $valor) {
                    if ($indice == 'completo_tema') {
                        ...
                        if ($indice == 'imagen_artista') {
                            ...
                            echo '
                                <td>
                                <img class="temaplus-img" src="', $valor, '" alt="Theme image">
                                </td>';
                        }
                    } else {
                        echo '<td>', $valor, '</td>';
            ...
            echo '<p class="notemas-p">No hay licencias disponibles</p>';
        }
        echo '</fieldset>';
    }
